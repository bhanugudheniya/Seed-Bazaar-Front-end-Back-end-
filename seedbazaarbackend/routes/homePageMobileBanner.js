...
const HomepageMobileBanner = require('../models/homePageMobileBanner');
...
router.post('/', upload.single('file'), async (req, res) => {
    try {
        if (res.req.file) {
            ...
            const banner = await new HomepageMobileBanner(req.body).save();
...
router.patch('/setBannerActive/:id', async (req, res) => {
    try {
        const banner = await HomepageMobileBanner.findByIdAndUpdate(req.params.id, { active: true }).exec();
        if (banner)
...
router.patch('/setBannerInactive/:id', async (req, res) => {
    try {
        const banner = await HomepageMobileBanner.findByIdAndUpdate(req.params.id, { active: false }).exec();
        if (banner)
...
router.get('/', async (req, res) => {
    try {
        const bannerArr = await HomepageMobileBanner.find().exec();
...
router.get('/getActiveBanner', async (req, res) => {
    try {
        const bannerArr = await HomepageMobileBanner.find({ active: true }).exec();
        res.json({ message: "Banner", data: bannerArr, success: true })
    }
    catch (err) {
        console.log(err);
...
router.delete('/removeBanner/:id/:filename', async (req, res) => {
    try {
        const bannerRemove =await HomepageMobileBanner.findByIdAndRemove(req.params.id).exec() 
        if(bannerRemove){
            dir = path.join(__dirname, '..', 'public', 'uploads', req.params.filename);
